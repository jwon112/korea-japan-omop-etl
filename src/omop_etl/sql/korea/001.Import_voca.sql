...
/* Athena vocabulary files are UTF-8. Match the declared character length
   while avoiding byte-length failures on multilingual names. */
DECLARE @unicode_sql NVARCHAR(MAX) = N'';
...
;WITH targets AS (
  ...
  WHERE s.name = N'dbo'
    AND t.name IN (
      N'CONCEPT', N'CONCEPT_SYNONYM', N'CONCEPT_RELATIONSHIP',
      N'CONCEPT_ANCESTOR', N'DRUG_STRENGTH', N'VOCABULARY',
      N'DOMAIN', N'CONCEPT_CLASS', N'RELATIONSHIP',
